...
namespace CNCEmu
{
    ...
    {
        public static List<Tdf> NotifyServerGameSetup(Packet p, User pi)
        {
            List<Tdf> input = Blaze.ReadPacketContent(p);

            uint t = Blaze.GetUnixTimeStamp();
            pi.ActiveGame.GNAM = ((TdfString)input[4]).Value;
            pi.ActiveGame.GSET = ((TdfInteger)input[5]).Value;
            pi.ActiveGame.VOIP = ((TdfInteger)input[22]).Value;
            pi.ActiveGame.VSTR = ((TdfString)input[23]).Value;
